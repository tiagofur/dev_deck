...
- Name: smoke.spec.ts >> DevDeck — desktop renderer E2E >> 2. capture item: opens modal, submits, sees the new card
...
Call log:
  - waiting for getByRole('button', { name: /capture/i })
...
```ts
  ...
  2   | 
  3   | test.describe('DevDeck — desktop renderer E2E', () => {
  4   |   test.beforeEach(async ({ page }) => {
  5   |     page.on('console', (msg) => {
  6   |       console.log(`Browser [${msg.type()}]: ${msg.text()}`)
  7   |     })
  8   |     page.on('pageerror', (err) => {
  9   |       console.error(`Browser Error: ${err.message}`)
  10  |     })
  11  |     await page.goto('/')
  12  |   })
  13  | 
  14  |   test('0. network check: can reach backend authenticated via proxy', async ({ page }) => {
  15  |     const apiUrl = '/api/auth/me'
  16  |     const apiToken = 'test-api-token'
  17  |     const response = await page.evaluate(async ({ url, token }) => {
  18  |       try {
  19  |         const controller = new AbortController()
  20  |         const timeoutId = setTimeout(() => controller.abort(), 5000)
