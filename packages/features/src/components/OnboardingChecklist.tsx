...
export function OnboardingChecklist() {
  ...
  return (
    <div className="max-w-md w-full bg-bg-card border-3 border-ink shadow-hard p-6 text-left">
      <h3 className="font-display font-black uppercase text-sm tracking-widest mb-4">{t('onboarding.title')}</h3>
      ...
      <div className="space-y-4">
        ...
        <CheckItem 
            ...
            action={<Button size="sm" variant="secondary" onClick={() => window.open('https://chrome.google.com/webstore', '_blank')}><Chrome size={12} className="mr-1" /> {t('settings.enable_button')}</Button>}
        />
        {!isDesktop && (
          <CheckItem 
            done={false} 
            ...
            action={<Button size="sm" variant="secondary" onClick={() => window.open('/download', '_blank')}><Monitor size={12} className="mr-1" /> {t('common.download')}</Button>}
          />
        )}
        <CheckItem 
            done={false} 
