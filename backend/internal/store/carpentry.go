...
func (s *Store) UpdateMaterial(ctx context.Context, id uuid.UUID, in UpdateMaterialInput) (*Material, error) {
	...
	if in.UnitCostMXN != nil {
		add("unit_cost_mxn", *in.UnitCostMXN)
	}
	if in.Supplier != nil {
		add("supplier", *in.Supplier)
	}
	if in.Notes != nil {
		add("notes", *in.Notes)
	}
...
func (s *Store) UpdateCarpentryProject(ctx context.Context, id uuid.UUID, in UpdateProjectInput) (*Project, error) {
	...
	if in.ProjectDate != nil {
		add("project_date", *in.ProjectDate)
	}
	if in.QuoteValidUntil != nil {
		add("quote_valid_until", *in.QuoteValidUntil)
	}
	if in.Notes != nil {
		add("notes", *in.Notes)
	}
	if in.MarginPercent != nil {
		add("margin_percent", *in.MarginPercent)
	}
	if in.AdvancePercent != nil {
		add("advance_percent", *in.AdvancePercent)
	}
	if len(sets) == 0 {
		return s.GetCarpentryProject(ctx, id)
	}
	sets = append(sets, "updated_at = NOW()")
	args = append(args, id)
	idIdx := len(args)
...
func (s *Store) CreateEnvironment(ctx context.Context, projectID uuid.UUID, in CreateEnvironmentInput) (*ProjectEnvironment, error) {
	if _, err := s.GetCarpentryProject(ctx, projectID); err != nil {
		return nil, err
	}
...
func (s *Store) UpdateEnvironment(ctx context.Context, id uuid.UUID, in UpdateEnvironmentInput) (*ProjectEnvironment, error) {
	if err := s.authorizeEnvironment(ctx, id); err != nil {
		return nil, err
	}
	sets, args := []string{}, []any{}
	add := func(name string, v any) {
...
func (s *Store) CreateFurniture(ctx context.Context, envID uuid.UUID, in CreateFurnitureInput) (*FurnitureItem, error) {
	if err := s.authorizeEnvironment(ctx, envID); err != nil {
		return nil, err
	}
	waste := 0.10
	if in.WastePercent != nil {
		waste = *in.WastePercent
	}
...
func (s *Store) UpdateFurniture(ctx context.Context, id uuid.UUID, in UpdateFurnitureInput) (*FurnitureItem, error) {
	if err := s.authorizeFurniture(ctx, id); err != nil {
		return nil, err
	}
	sets, args := []string{}, []any{}
	add := func(name string, v any) {
	...
	if in.WidthMM != nil {
		add("width_mm", *in.WidthMM)
	}
	if in.HeightMM != nil {
		add("height_mm", *in.HeightMM)
	}
	if in.DepthMM != nil {
		add("depth_mm", *in.DepthMM)
	}
	if in.WastePercent != nil {
		add("waste_percent", *in.WastePercent)
	}
	if in.Position != nil {
		add("position", *in.Position)
	}
	if len(sets) == 0 {
		return s.getFurniture(ctx, id)
	}
	sets = append(sets, "updated_at = NOW()")
	args = append(args, id)
...
func (s *Store) CreateCostLine(ctx context.Context, furnitureID uuid.UUID, in CreateCostLineInput) (*CostLine, error) {
	if err := s.authorizeFurniture(ctx, furnitureID); err != nil {
		return nil, err
	}
	if in.MaterialID != nil && (in.Name == "" || in.Unit == "" || in.UnitCostSnapshot == nil) {
		...
		if in.UnitCostSnapshot == nil {
			in.UnitCostSnapshot = &m.UnitCostMXN
		}
	}
	if in.Quantity == 0 {
		in.Quantity = 1
...
func (s *Store) UpdateCostLine(ctx context.Context, id uuid.UUID, in UpdateCostLineInput) (*CostLine, error) {
	if err := s.authorizeCostLine(ctx, id); err != nil {
		return nil, err
	}
	sets, args := []string{}, []any{}
	add := func(name string, v any) {
	...
	if in.MaterialID != nil {
		add("material_id", *in.MaterialID)
	}
	if in.LineType != nil {
		add("line_type", *in.LineType)
	}
	if in.Name != nil {
		add("name", *in.Name)
	}
	...
	if in.UnitCostSnapshot != nil {
		add("unit_cost_snapshot", *in.UnitCostSnapshot)
	}
	if in.Notes != nil {
		add("notes", *in.Notes)
	}
	if len(sets) == 0 {
		return s.getCostLine(ctx, id)
	}
	sets = append(sets, "updated_at = NOW()")
	args = append(args, id)
